...
vi.mock("../../mobile/src/lib/captures", () => ({ listCaptures: async (session: unknown) => ({ session, data: { items: [] } }), mapCaptureRecordToHistoryItem: vi.fn() }));
...
const selectedSession = { key: "acid-14", plant: "Acid Plant", session: "14.00", slot: 1 } as TodaySessionItem;
...
beforeEach(() => {
  ...
  api.requestResponseWithSession.mockResolvedValue({ session, response: new Response("image", { headers: { "content-type": "image/jpeg" } }) });
});
afterEach(async () => {
...
async function mount(strict = false) {
  await act(async () => {
    const element = createElement(CaptureScreen, { session, selectedSession, operatorName: "acid", onSessionUpdate: () => undefined });
...
it("automatically starts the plant session and enables capture after preview", async () => {
  await mount();
  expect(api.requestWithSession.mock.calls.filter((call) => call[1] === "/camera/session")).toHaveLength(1);
...
it("releases a late created lease after navigating away", async () => {
  ...
  api.requestWithSession.mockImplementationOnce(() => new Promise((resolve) => { finish = resolve; }));
  await mount();
  await act(async () => renderer.unmount());
  renderer = undefined;
  await act(async () => { finish({ session, data: lease }); });
...
it("ALL account stays idle without selection and switches cameras with the selected session plant", async () => {
  ...
  api.requestWithSession.mockImplementation(async (_session, _path, options) => {
    ...
    return { session: allSession, data: { ...lease, plant, deviceId: plant === "Acid Plant" ? 7 : 8 } };
  });
  await act(async () => { renderer = create(createElement(CaptureScreen, { ...props, selectedSession: null })); });
  expect(api.requestWithSession).not.toHaveBeenCalled();
  expect(renderer.root.findByProps({ "aria-label": "Capture image" }).props.disabled).toBe(true);
  await act(async () => { renderer.update(createElement(CaptureScreen, { ...props, selectedSession })); });
  expect(renderer.root.findByProps({ "aria-label": "Capture image" }).props.disabled).toBe(false);
  await act(async () => { renderer.update(createElement(CaptureScreen, { ...props, selectedSession: { ...selectedSession, key: "chloride-14", plant: "Chloride Plant" } })); });
  ...
  expect(api.requestResponseWithSession.mock.calls.at(-1)?.[1]).toContain("deviceId=8&plant=Chloride+Plant");
});

it("ALL My Device requests selection without guessing a camera", async () => {
  const user = { ...session.user, plant: "ALL" };
  await act(async () => { renderer = create(createElement(MyDeviceScreen, { session: { ...session, user }, user, onSessionUpdate: () => undefined, onSignOut: () => undefined, onOpenSessions: () => undefined })); });
  expect(api.requestWithSession).not.toHaveBeenCalled();
...
it("ALL My Device reads only the selected plant camera", async () => {
  const user = { ...session.user, plant: "ALL" };
  api.requestWithSession.mockImplementation(async () => ({ session: { ...session, user }, data: {
    devices: [{ code: "acid", plant: "Acid Plant", isActive: true }, { code: "chloride", plant: "Chloride Plant", isActive: true }], edge: { connected: true },
  } }));
  await act(async () => { renderer = create(createElement(MyDeviceScreen, { session: { ...session, user }, user, selectedPlant: "Chloride Plant", onSessionUpdate: () => undefined, onSignOut: () => undefined })); });
  expect(api.requestWithSession.mock.calls.map((call) => call[1])).toEqual(["/devices", "/devices/chloride/status"]);
});
